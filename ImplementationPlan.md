...
## 課題点
- [ ] 訓練データとテストデータで `track-id` が一致する曲の予測値を上書きしている。
...
## 開発方針
- [ ] `popularity` が0のデータも多くあるので、まずは「`popularity` が0か、それ以外か」という分類タスクにかける。その後0以外と分類された楽曲に対して回帰タスクを回す、というモデルを作成し、性能がどう変わるか検証。　現状では、「1未満を0、それ以外は1~100」とデータをクリップしている。
...
- [ ] `album_name`や`main_artist`を特徴量から除いてみる。
